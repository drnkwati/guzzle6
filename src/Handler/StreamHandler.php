...
class StreamHandler
{
    ...
    private function createResponse(
        ...
        $headers = \GuzzleHttp\Guzzle\headers_from_lines($hdrs);
    ...
    private function checkDecode(array $options, array $headers, $stream)
    {
        ...
        if (!empty($options['decode_content'])) {
            $normalizedKeys = \GuzzleHttp\Guzzle\normalize_header_keys($headers);
    ...
    public function __add_proxy(RequestInterface $request, &$options, $value, &$params)
    {
        if (!is_array($value)) {
            ...
            if (isset($value[$scheme])) {
                if (!isset($value['no'])
                    || !\GuzzleHttp\Guzzle\is_host_in_noproxy(
    ...
    public function __add_verify(RequestInterface $request, &$options, $value, &$params)
    {
        if ($value === true) {
            ...
            if (PHP_VERSION_ID < 50600) {
                $options['ssl']['cafile'] = \GuzzleHttp\Guzzle\default_ca_bundle();
            }
    ...
    public function __add_debug(RequestInterface $request, &$options, $value, &$params)
    {
        ...
        $value = \GuzzleHttp\Guzzle\debug_resource($value);

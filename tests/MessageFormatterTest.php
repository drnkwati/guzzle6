...
use GuzzleHttp\Guzzle\Exception\RequestException;
use GuzzleHttp\Psr7\Request;
use GuzzleHttp\Psr7\Response;
use GuzzleHttp\Guzzle\MessageFormatter;

/**
 * @covers \GuzzleHttp\Guzzle\MessageFormatter
 */
class MessageFormatterTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $request = new Request('PUT', '/', array('x-test' => 'abc'), \GuzzleHttp\Psr7\stream_for('foo'));
        $response = new Response(200, array('X-Baz' => 'Bar'), \GuzzleHttp\Psr7\stream_for('baz'));
        $err = new RequestException('Test', $request, $response);

        return array(
            array('{request}', array($request), \GuzzleHttp\Psr7\str($request)),
            array('{response}', array($request, $response), \GuzzleHttp\Psr7\str($response)),
            array('{request} {response}', array($request, $response), \GuzzleHttp\Psr7\str($request) . ' ' . \GuzzleHttp\Psr7\str($response)),
            // Empty response yields no value
            array('{request} {response}', array($request), \GuzzleHttp\Psr7\str($request) . ' '),
